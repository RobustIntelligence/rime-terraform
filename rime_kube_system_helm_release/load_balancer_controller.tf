...
module "iam_assumable_role_with_oidc_for_load_balancer_controller" {
  ...
  role_name = "aws_load_balancer_controller_${var.resource_name_suffix}"

  tags = {
    Role = "aws_load_balancer_controller_${var.resource_name_suffix}"
  }

  provider_url = var.oidc_provider_url

  role_policy_arns = [
    aws_iam_policy.aws_load_balancer_controller_iam_policy[0].arn,
  ]

  oidc_fully_qualified_subjects = ["system:serviceaccount:kube-system:rime-kube-system-aws-load-balancer-controller"]
